...
-- -----------------------------------------------------
-- Table `unomasdb`.`persistence_logins`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `unomasdb`.`persistence_logins` (
  `user_num` INT NOT NULL COMMENT '자동로그인위한 회원번호정보 가져오기',
  `series` VARCHAR(64) NULL COMMENT '고유 시리즈넘버',
  `token` VARCHAR(64) NULL COMMENT '저장된 토큰정보',
  `last_used` TIMESTAMP NULL DEFAULT current_timestamp COMMENT '마지막 접속정보',
  PRIMARY KEY (`user_num`),
  CONSTRAINT `fk_user_num`
    FOREIGN KEY (`user_num`)
    REFERENCES `unomasdb`.`users` (`user_num`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
COMMENT = '자동로그인을 위한 정보저장위한 테이블';
