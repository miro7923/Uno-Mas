...
CREATE TABLE IF NOT EXISTS `unomasdb`.`products` (
  ...
  `prod_expire` VARCHAR(100) NOT NULL COMMENT '상품 유통기한',
  ...
  `prod_sellunit` VARCHAR(10) NOT NULL COMMENT '상품 판매단위\n예) 1개/ 1팩/ 1박스…',
  `prod_explain` VARCHAR(50) NOT NULL COMMENT '상품 요약 설명\n예) 청경채: 아삭하고 부드러운',
  `prod_packing` VARCHAR(20) NOT NULL COMMENT '상품 포장타입\n예) 냉장/스티로폼, 냉동/스티로폼',
...
USE `unomasdb`$$
CREATE DEFINER = CURRENT_USER TRIGGER `unomasdb`.`products_BEFORE_INSERT` BEFORE INSERT ON `products` FOR EACH ROW
BEGIN
IF 
  new.prod_packing != '냉장/스티로폼' and 
  new.prod_packing != '냉동/스티로폼'
THEN
 signal sqlstate 'ERROR' 
 set message_text = '*냉장/스티로폼* *냉동/스티로폼* 만 가능';
END IF;
END$$
